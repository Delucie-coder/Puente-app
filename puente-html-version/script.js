...
if (loginForm){
  ...
  if (resp.ok){ const j = await resp.json(); const user = j.user || { email }; try{ sessionStorage.setItem('puente_user', JSON.stringify(user)); sessionStorage.setItem('puente_role', user.role || role); if (j.token) sessionStorage.setItem('puente_token', j.token); else if (user.token) sessionStorage.setItem('puente_token', user.token); }catch(e){} window.location.href='dashboard.html'; return; }
      // if not ok and status 401, offer to register
      if (resp.status === 401){
        // attempt registration
        const reg = await fetch('/api/register', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ email, name: username || 'Guest', password, role }) });
  if (reg.ok){ const jr = await reg.json(); const user = jr.user; try{ sessionStorage.setItem('puente_user', JSON.stringify(user)); sessionStorage.setItem('puente_role', user.role || role); if (jr.token) sessionStorage.setItem('puente_token', jr.token); else if (user.token) sessionStorage.setItem('puente_token', user.token); }catch(e){} window.location.href='dashboard.html'; return; }
